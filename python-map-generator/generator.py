# ---------------------------
# Island Map Generator Core
# 岛屿地图生成器核心逻辑
# ---------------------------
#
# A refactored version of simple_demo.py with separated concerns:
# - Generator functions for island and environment generation
# - No Pygame dependencies (can be used headlessly)
#
# 重构版本的 simple_demo.py，职责分离：
# - 生成器函数用于岛屿和环境生成
# - 无 Pygame 依赖（可无头运行）
...
def parse_radii(text, num_islands):
    ...
    if not parts:
        # 如果为空，生成与岛屿数量匹配的默认半径 / Generate default radii matching island count
        return [35 - i * 4 for i in range(num_islands)]
    ...
    if len(radii) != num_islands:
        raise ValueError(f"Number of radii ({len(radii)}) must match number of islands ({num_islands}), or be a single value to repeat.")
    return radii
...
def generate_islands(width, height, radii, rng):
    ...
    # 连接岛屿与桥梁，使整个陆地块相连 / Connect islands with bridges
...
def generate_map(
    ...
    num_islands=1,
    radii="35",
    environments="forest:5, plains:3, water:2, mountain:1",
    base_switch=0.03,
    switch_growth=0.0025,
    adjacency_bias=0.35,
    cohesion_bias=0.20,
    ...
    """
    Generate a complete island map with environment regions.
    生成完整的岛屿地图及环境区域。

    Args:
        width: 地图宽度 / Map width
        height: 地图高度 / Map height
        seed: 随机种子（可选）/ Random seed (optional)
        num_islands: 岛屿数量 / Number of islands
        radii: 岛屿半径（逗号分隔或单个值）/ Island radii (comma-separated or single value)
        environments: 环境类型和权重 / Environment types and weights
        base_switch: 基础切换概率 / Base switch probability
        switch_growth: 每个单元格增长的切换概率 / Switch growth per cell
        adjacency_bias: 邻近偏差 / Adjacency bias
        cohesion_bias: 内聚性偏差 / Cohesion bias

    Returns:
        tuple: (land, env_map, env_colors, legend, seed)
            - land: 2D布尔网格 / 2D boolean grid
            - env_map: 环境映射 {(x,y): env_name} / Environment map
            - env_colors: 颜色映射 / Color mapping
            - legend: 图例 / Legend
            - seed: 使用的随机种子 / Random seed used
    """
    # 初始化随机数生成器 / Initialize RNG
    if seed is None:
        seed = int(random.random() * 100000)
    rng = random.Random(seed)

    # 解析半径 / Parse radii
    radii_list = parse_radii(radii, num_islands)

    # 解析并归一化环境权重 / Parse and normalize environment weights
    env_items = parse_envs(environments)
    env_weights = normalize_weights(env_items)

    # 生成岛屿 / Generate islands
    land = generate_islands(width, height, radii_list, rng)

    # 生成环境地图 / Generate environment map
    ...
    # 构建图例 / Build legend
    legend = {"Air/Vacuum": (0, 0, 0)}
    legend.update(env_colors)

    return land, env_map, env_colors, legend, seed
